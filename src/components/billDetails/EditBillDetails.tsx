...
interface EditBillDetailsProps {
	bill: BillDetail;
	onCancel: () => void;
			...
			<div className="flex flex-col lg:flex-row gap-10">
				...
				<div className="flex-1 flex flex-col gap-8">
					...
					{formData.products.map((product) => (
						<div
							key={product.id}
							className="bg-[var(--color-bg-card)] rounded-2xl border border-[var(--color-border)] p-8 shadow-sm"
						>
							<div className="flex items-center gap-4 mb-8 pb-6 border-b border-[var(--color-border)]">
								...
								<h3 className="text-lg font-bold text-[var(--color-text-heading)] opacity-90">
									{product.name}
								</h3>
							</div>
